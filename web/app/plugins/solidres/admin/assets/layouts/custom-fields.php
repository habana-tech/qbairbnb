...
if ( isset( $id ) ) {
	$custom_fields = new SR_Custom_Field( array( 'id' => (int) $id, 'type' => 'asset' ) );
} else {
	$custom_fields = new SR_Custom_Field( array( 'id' => null, 'type' => 'asset' ) );
}
...
if( ! empty( $custom_fields_array ) ) {
	foreach ( $custom_fields_array as $group_key => $group_fields ) {
		...
		$list_table_content .= '<td><input type="button" class="button" id="add_new_field" value="Add New Field" /></td>';
...
<div id="asset_custom_fields" class="postbox closed open">
	...
	<div class="inside">
		<div class="add_tabs_dynamic">
			<input type="button" class="button" id="add_new_group" value="<?php _e( 'Add New Group', 'solidres' ); ?>"/>
			<input type="text" id="group_name" name="ignfield[]" class="" value="" placeholder="<?php _e( 'Enter group name', 'solidres' ); ?>">
		</div>
